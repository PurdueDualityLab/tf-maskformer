...
class PanopticQualityV2(tf.keras.metrics.Metric):
  ...
  def update_state(
    ...
    # tf.print("category_mask shape :", category_mask.shape)
    # tf.print("instance_mask shape :", instance_mask.shape)
    # tf.print("gt_category_mask shape :", gt_category_mask.shape)
    # tf.print("gt_instance_mask shape :", gt_instance_mask.shape)
  ...
  def transpose_in_chunks(self, tensor, perm):
    batch_size = tensor.shape[0]
    for chunk_size in range(batch_size, 0, -1):
        if batch_size % chunk_size == 0:
            break
    chunks = tf.split(tensor, num_or_size_splits=chunk_size, axis=0)
  ...
  def _update_thing_classes(
    ...
    flattened_binary_masks = self.transpose_in_chunks(flattened_binary_masks, [0, 2, 1])
