...
def position_embedding_sine_old(attention_mask,
							num_pos_features=256,
							temperature=10000.,
							normalize=True,
							scale=2 * math.pi):
	...
	# <tf.float>[batch_size, height, width]
	attention_mask = tf.cast(attention_mask, tf.float32)
	mask = tf.cast(tf.zeros((tf.shape(attention_mask)[0], tf.shape(attention_mask)[1], tf.shape(attention_mask)[2])), tf.bool)
	not_mask = tf.cast(~mask, tf.float32)

	row_embedding = tf.cumsum(not_mask, 1)
	col_embedding = tf.cumsum(not_mask, 2)
...
class TransformerFPN(tf.keras.layers.Layer):
	...
	def call(self, multilevel_features, image):
		...
		pos_embed = position_embedding_sine(mask[:, :, :, 0], num_pos_features=self._fpn_feat_dims)
		
		# with options({"layout_optimizer": False}):
		features = tf.reshape(features, [tf.shape(features)[0], -1, tf.shape(features)[-1]]) # (2, 400, 2048)
		pos_embed = tf.reshape(pos_embed, [tf.shape(pos_embed)[0], -1, tf.shape(pos_embed)[-1]]) # (2, 400, 256)
		transformer = self._transformer_encoder(features, attention_mask = None, pos_embed = pos_embed)
		transformer = tf.reshape(transformer, [tf.shape(transformer)[0], tf.shape(feat)[1], tf.shape(feat)[2], tf.shape(transformer)[-1]])
