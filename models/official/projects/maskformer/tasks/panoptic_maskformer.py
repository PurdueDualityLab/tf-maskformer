...
class PanopticTask(base_task.Task):
	...
	def build_losses(self, class_prob_outputs, mask_prob_outputs, class_targets, mask_targets, aux_ouptuts=None):
		outputs = {"pred_logits": class_prob_outputs, "pred_masks": mask_prob_outputs, "aux_outputs": aux_ouptuts}
	...
	def train_step(self, inputs: Tuple[Any, Any],model: tf.keras.Model, optimizer: tf.keras.optimizers.Optimizer, metrics: Optional[List[Any]] = None) -> Dict[str, Any]:
		...
		##########################################################################
			# TODO : unpack the outputs and losses
			total_loss, cls_loss, focal_loss, dice_loss = self.build_losses(outputs=outputs, labels=labels, aux_losses=model.losses)
